export class DataChannelHandler {
    static setupDataChannel(instance, channel) { // `instance` is SyncFiles
        channel.onopen = () => {
            ...
            console.log("SyncFiles confirmed data channel is open.");
        };
        ...
        channel.onclose = () => {
            ...
            console.log("SyncFiles confirmed data channel is closed.");
        };
        
        channel.onerror = (event) => {
            const error = event.error;
            if (error && error.name === 'OperationError' && error.message.includes('User-Initiated Abort')) {
                console.log('Data channel closed intentionally by a peer.', event);
            } else {
